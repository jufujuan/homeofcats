 ...
 */
public class ReChooseFraRVAdapter extends RecyclerView.Adapter<CommonViewHolder> {

    private ChooseForumBean datas;
    private Context mContext;


    public ReChooseFraRVAdapter(Context context) {
        mContext = context;
    }


    public void setDatas(ChooseForumBean datas) {
        this.datas = datas;
...
//    }
    ...
    public void onBindViewHolder(final CommonViewHolder holder, final int position) {
        ...
        holder.setText(R.id.item_re_news_center_play_count,datas.getResult().getList().get(position).getPostusername());
        holder.setText(R.id.item_re_news_center_date,datas.getResult().getList().get(position).getReplycounts()+"回帖");
        holder.setImg(R.id.item_re_news_center_logo,datas.getResult().getList().get(position).getSmallpic()
                ,2000,2000, ScaleType.CENTER, Config.ARGB_8888);
    }
